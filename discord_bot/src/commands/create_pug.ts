  ...
  async execute(interaction: ChatInputCommandInteraction) {
    try {
      ...
      if (!team1Ids.length || !team2Ids.length) {
        return interaction.reply({
          content: "You must mention at least one player in each team (e.g. @user).",
          flags: 64,
        });
      }
      ...
      if ((interaction as any).deferred || (interaction as any).replied) {
        await (interaction as any).followUp({ content: "Something went wrong creating the PUG.", flags: 64 });
      } else {
        await (interaction as any).reply({ content: "Something went wrong creating the PUG.", flags: 64 });
      }
    }
  },
};